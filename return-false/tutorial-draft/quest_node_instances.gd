...
func set_state(state, quest_id):
	var qNode = quests.get_node(quest_id)
	if( not qNode.is_in_group("active_quests") ):
		return
	else:
		qNode.set("currentState", state)

func get_state(quest_id):
	var qNode = quests.get_node(quest_id)
	if( qNode.get("currentState")):
		return qNode.get("currentState")
	else:
		return null
...
func _ready():
	...
	mui = get_node("scene/message-ui")
	
	activate_quest("tutorial")
	print(get_state("tutorial"))
	
	terminal = npcs.get_node("terminal_1/branches/tutorial")
...
func _on_clickable_pressed():
	if( is_active("tutorial") ):
		terminal.interact("20")
		#mui.open()
		#m#ui.make_dialogue(terminal.text_at_state("20"))
